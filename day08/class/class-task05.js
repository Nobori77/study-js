...
class Airbnb {
  ...
  static hotelPrice = 700000

  constructor(hotelName, hotelMoney){
    this.hotelName = hotelName;
    this.hotelMoney = hotelMoney;
  }
    
  trip(member){
    if(member.memberMoney < Airbnb.hotelPrice){
      console.log("예약금이 부족합니다.")
    }
    member.memberMoney -= Airbnb.hotelPrice
    this.hotelMoney += Airbnb.hotelPrice

    if(member.memberClass === "브론즈"){
      member.memberMileage += Airbnb.hotelPrice * 0.01
    } else if (member.memberClass === "실버"){
      member.memberMileage += Airbnb.hotelPrice * 0.03
    } 
    else if(member.memberClass === "골드"){
      member.memberMileage += Airbnb.hotelPrice * 0.05
    } else if(member.memberClass === "플레티넘"){
      member.memberMileage += Airbnb.hotelPrice * 0.1
    } else{""}
  }
}
...
class Member{ 
   constructor(memberName, memberMoney, memberMileage, memberClass){
...
const noh = new Member("노민균", 1_200_000, 0, "플레티넘")
const sin = new Airbnb("신라호텔", 10000000)
